...
The [ENSEMBL website](http://www.ensembl.org/) provides references for different organisms; [here](http://www.ensembl.org/Drosophila_melanogaster/Info/Index) for the fruitfly. Former releases can be retrieve from the [ftp server](ftp://ftp.ensembl.org/pub/) or browse from the [`archives`](http://www.ensembl.org/info/website/archives/index.html) part of the ENSEMBL website.
